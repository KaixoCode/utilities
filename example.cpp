...
int main() {
    ...
    using pack = kaixo::pack<my_complicated_tuple>;
    using edit = pack::remove<float>::take<2>::prepend<char>;
    using back_to_tuple = edit::as<std::tuple>;
    static_assert(std::same_as<back_to_tuple, std::tuple<char, int, double>>);
}